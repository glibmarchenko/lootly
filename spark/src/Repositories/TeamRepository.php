...
class TeamRepository implements TeamRepositoryContract
{
    ...
    {
        return CacheRepository::rememberCacheByTag(
            User::class . $id,
            'TeamRepository@Find',
            function() use($id){
    ...
    {
        return CacheRepository::rememberCacheByObject(
            $user,
            'TeamRepository@ForUser',
            function() use($user) { return $user->teams()->with('owner')->get(); }
        );
    }
