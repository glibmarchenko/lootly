...
class OrdersPaidJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        Log::info('Paid order at '.$shopDomain.'.');
        ...
        if (isset($shopifyIntegration) && $shopifyIntegration) {
            ...
            if ($targetMerchants && count($targetMerchants)) {
                foreach ($targetMerchants as $merchant) {
                    ...
                    if (! isset($integrationSettings['order_settings']['reward_status']) || strtolower($integrationSettings['order_settings']['reward_status']) != 'paid') {
