...
class RefundsCreateJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        if ($merchant) {
            ...
            if (! isset($integrationSettings['order_settings']['subtract_status']) || ! in_array(strtolower($integrationSettings['order_settings']['subtract_status']), [
                    'refunded',
                    'any',
                ])) {
            ...
            try {
                                            ...
                                            if ($point && $point->point_value > 0) {
                                                ...
                                                try {
                                                    ...
                                                    if (isset($goalOrdersAction->goal) && intval($ordersCount) < intval($goalOrdersAction->goal)) {
                                                        try {
                                                            $this->points->clearEntity();
                                                            $deducted = $this->points->rollbackPoints($point->id, ['comment' => (isset($comment) ? $comment : '')]);
                                                            $deductedPoints[] = $deducted->id;
                                                        } catch (\Exception $e) {
                                                            $errors[] = [
                                                                'id'      => $point->id,
                ...
                Log::info('Error during order (#'.($this->data->id ?? '').') refunding for merchant #'.$merchant->id.': '.$e->getMessage());
            }
        } else {
