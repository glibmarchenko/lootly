...
class EmailNotificationSettingsService
{
    ...
    public function getTierIconUrl(Tier $tier, $iconTag = null, $notificationType = null)
    {
        if (trim($tier->image_url)) {
            return $tier->image_url;
        }
        ...
        $tierIconPngRelativePath = '/images/icons/lootly/'.$iconName.'.png';

        $tierIconPngPath = public_path().$tierIconPngRelativePath;

        if (! $iconTag) {
            return url($tierIconPngRelativePath);
        }

        $iconColor = '';

        if ($iconTag == 'vip-tier-icon') {
            $iconColor = $tier->default_icon_color;
        }

        if (! trim($iconColor) && ! $notificationType) {
            return url($tierIconPngRelativePath);
        }
        ...
        if (! trim($iconColor)) {
            try {
                $notificationSettings = $this->notificationSettings->withCriteria([
                    new ByMerchant($tier->merchant_id),
                ])->findWhereFirst([
                    'notification_type' => $notificationType,
                ]);
        ...
        if (trim($iconColor)) {
            if (isset($iconColor[0]) && $iconColor[0] == '#') {
                $iconColor = $this->convert_hex2rgba($iconColor);
            }
        }
        ...
        if (preg_match('/rgba?\(([0-9]+) *\, *([0-9]+) *\, *([0-9]+) */', $iconColor, $rgb_matches)) {
            ...
            // Colorize image
            $img = $this->switchPngImageColor($tierIconPngPath, $r, $g, $b);
            if ($img) {
                // Save to S3
                try {
                    Storage::disk('s3')->put($path, $this->pngImageData($img), 'public');
    ...
    protected function convert_hex2rgba($color, $opacity = false)
    {
        $default = 'rgb(0,0,0)';
