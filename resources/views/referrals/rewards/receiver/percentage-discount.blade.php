@extends('layouts.app')
@section('title', 'Percentage off discount')
@section('content')
            ...
            <div class="row p-b-10 section-border-bottom">
                ...
                <div class="col-md-12">
                    <h3 class="page-title m-t-0 color-dark pull-left">
                        Receiver Reward: <span v-text="form.defaultRewardName"></span>
                    </h3>
                        ...
                        <div :class="{ 'loading' : loading }" v-cloak>
                            <div class="row m-b-15">
                                <div class="col-md-12">
                                    ...
                                    <a href="{{route('display.email.referral.receiver-reward')}}"
    ...
    <script type="module">
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            reward_id: '',
            defaultRewardName: 'Percentage off discount',
            program: {
              name: 'They will receive',
              nameDefault: 'They will receive',
              rewardType: 'Percentage off',
              rewardText: '',
              rewardTextDefault: '{reward-value}% off discount {min-value}',
              emailTextDefault: 'You just unlocked a <strong>{reward-text}</strong> discount at {company-name}. Below is your coupon code to use on your next purchase.',
              icon: null,
              icon_name: 'icon-percentage',
              reward_icon: null,
              status: true,
            },
            reward: {
            ...
            reward_name_tags: [],
            reward_text_tags: ['{currency}', '{reward-value}', '{min-value}'], // third tag is using in min value generating
            reward_email_tags: ['{customer}', '{reward-text}', '{company}', '{company-website}', '{coupon-code}'],
            iconPreview: '',
            emailNotification: 1,
          },
          loading: true,
          rewardEmailTextPreview: '',
          icon_default_class: 'icon-percentage',
        ...
        methods: {
          ...
          getRewards: function () {
            let _this = this
            axios.get('/referrals/rewards/receiver/get/' + this.form.defaultRewardName).then((response) => {
          ...
          saveAction: function () {
            this.loading = true
            axios.post('/referrals/rewards/receiver/store', this.form).then((response) => {
          ...
          minOrderGen: function () {
            ...
            const minTag = this.form.reward_text_tags[2]
        ...
        computed: {
          ...
          rewardTextPreview: function () {
            ...
            return this.form.program.rewardText = this.form.program.rewardTextDefault
              .replace(/{currency}/g, this.form.reward.values ? this.merchantSettings.currency.selectedFormat : '')
              .replace(/{reward-value}/g, this.form.reward.values ? this.form.reward.values : '')
              .replace(/{min-value}/g, this.form.reward.minOrder ? `${this.merchantSettings.currency.selectedFormat}${this.form.reward.minOrder}` : '')
          },
        ...
        watch: {
          'form.reward.values': function () {
            this.form.program.rewardText = this.form.reward.values + '% off discount'
          }
        }
      })
