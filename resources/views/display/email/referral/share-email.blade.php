...
@section('title', 'Referral Share Email')
                            ...
                            <div class="loading-spinner" :class="{ 'loading' : loading || saving }" v-cloak>
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <input type="text" class="form-control m-b-5" name="subject"
    ...
    <script>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            subjectLine: '{sender-name} just sent you a {reward-name} at {company}',
            body: `<?php echo File::get(storage_path()."/email-notification/referral_share_email-editable.html"); ?>`,
            button: {
              text: 'Get My Coupon',
              color: '#022c82'
            },
          },
          tags: ['{email-body}', '{receiver-name}', '{sender-name}', '{reward-name}', '{reward-icon}', '{company}', '{button}'],
          icons: ['reward-icon'],
          config: {
        ...
        methods: {
          ...
          getData: function () {
            let comp = this
            comp.loading = true
            axios.get('/settings/display/email-notifications/referral/share-email').then((response) => {
          ...
          saveSetting () {
            ...
            if (!comp.saving) {
              ...
              axios.post('/settings/display/email-notifications/referral/share-email', formData).then((response) => {
                comp.saving = false
          ...
          sendTest () {
            ...
            if (!this.sending) {
              this.sending = true
              axios.post('/api/merchants/' + this.merchantId + '/email-notifications/referral/share-email/send-test-email', {
                'to_email': this.email
              }).then((response) => {
        ...
        computed: {
          bodyPreview () {
            ...
            var previewText = this.form.body.toString()
              .replace(/{email-body}/g, '{receiver-name}, <br/>{sender-name} just sent you a {reward-name} coupon for your next order at {company}.')
              .replace(/{receiver-name}/g, 'Joe')
              .replace(/{sender-name}/g, 'Ryan')
              .replace(/{company}/g, this.merchantSettings.name)
