...
@section('title', 'Referral Sender Reward')
    ...
    <script>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            subjectLine: 'You just earned {reward-name} at {company-name} for a referral',
            body: `<?php echo File::get(storage_path()."/email-notification/referral_sender_reward-editable.html"); ?>`,
            button: {
              text: 'Shop Now',
              color: '#022c82'
            },
          },
          tags: ['{customer}', '{reward-name}', '{company-name}', '{referral-name}', '{coupon-code}', '{reward-icon}', '{point-balance}', '{button}'],
          icons: ['reward-icon'],
          config: {
        ...
        methods: {
          ...
          getData: function () {
            let comp = this
            comp.loading = true
            axios.get('/settings/display/email-notifications/referral/sender-reward').then((response) => {
          ...
          saveSetting () {
            ...
            if (!comp.saving) {
              ...
              axios.post('/settings/display/email-notifications/referral/sender-reward', formData).then((response) => {
                comp.saving = false
          ...
          sendTest () {
            ...
            if (!this.sending) {
              this.sending = true
              axios.post('/api/merchants/' + this.merchantId + '/email-notifications/referral/sender-reward/send-test-email', {
                'to_email': this.email
              }).then((response) => {
        ...
        computed: {
          bodyPreview () {
            let button = '<table border="0" cellspacing="0" cellpadding="0"><tr><td class="text-button" style="padding: 14px 37px; border-radius: 5px; color:#ffffff; font-family:Arial,sans-serif; font-size:17px; line-height:21px; text-align:center; font-weight:bold;" bgcolor="' + this.form.button.color + '"><a href="#" target="_blank" class="link-white" style="color:#ffffff; text-decoration:none;"><span class="link-white" style="color:#ffffff; text-decoration:none;">' + this.form.button.text + '</span></a></td></tr></table>'
            let previewText = this.form.body.toString()
              .replace(/{customer}/g, 'Joe Smith')
              .replace(/{reward-earning-text}/g, 'You just earned a <strong>{reward-name}</strong> discount at {company-name}, as thanks for referring {referral-name} to make a purchase at our store.')
