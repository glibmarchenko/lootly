...
@section('title', 'Referral Receiver Reward')
    ...
    <script>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            subjectLine: 'You just unlocked a {reward-name} at {company-name}',
            body: `<?php echo File::get(storage_path()."/email-notification/referral_receiver_reward-editable.html"); ?>`,
            button: {
              text: 'Shop Now',
              color: '#022c82'
            },
          },
          tags: ['{customer}', '{reward-name}', '{company-name}', '{referral-name}', '{coupon-code}', '{reward-icon}', '{button}'],
          icons: ['reward-icon'],
          config: {
        ...
        methods: {
          ...
          getData: function () {
            let comp = this
            comp.loading = true
            axios.get('/settings/display/email-notifications/referral/receiver-reward').then((response) => {
          ...
          saveSetting () {
            ...
            if (!comp.saving) {
              ...
              axios.post('/settings/display/email-notifications/referral/receiver-reward', formData).then((response) => {
                comp.saving = false
          ...
          sendTest () {
            ...
            if (!this.sending) {
              this.sending = true
              axios.post('/api/merchants/' + this.merchantId + '/email-notifications/referral/receiver-reward/send-test-email', {
                'to_email': this.email
              }).then((response) => {
        ...
        computed: {
          bodyPreview () {
            ...
            var previewText = this.form.body.toString()
              ...
              .replace(/{reward-earning-text}/g, 'You just unlocked a <strong>{reward-name}</strong> discount at {company-name}. Below is your coupon code to use on your next purchase.')
              .replace(/{referral-name}/g, 'Ryan Haidinger')
