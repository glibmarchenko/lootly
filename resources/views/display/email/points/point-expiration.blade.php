...
@section('title', 'Point Expiration Reminder')
    ...
    <script>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            subjectLine: '{customer} you have reward {point-name} expiring soon at {company-name}',
            body: `<?php echo File::get(storage_path()."/email-notification/points_point_expiration-editable.html"); ?>`,
            button: {
              text: 'Use My Points',
              color: '#022c82'
            },
          },
          tags: ['{customer}', '{point-balance}', '{company-name}', '{expiration-time}', '{button}'],
          icons: [],
          config: {
        ...
        methods: {
          ...
          getData: function () {
            let comp = this
            comp.loading = true
            axios.get('/settings/display/email-notifications/points/point-expiration').then((response) => {
          ...
          saveSetting () {
            ...
            if (!comp.saving) {
              ...
              axios.post('/settings/display/email-notifications/points/point-expiration', formData).then((response) => {
                comp.saving = false
          ...
          sendTest () {
            ...
            if (!this.sending) {
              this.sending = true
              axios.post('/api/merchants/' + this.merchantId + '/email-notifications/points/point-expiration/send-test-email', {
                'to_email': this.email
              }).then((response) => {
        ...
        computed: {
          bodyPreview () {
            ...
            var previewText = this.form.body.toString()
              ...
              .replace(/{expiration-time}/g, '30 days')
