    ...
    <script>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            subjectLine: 'You just unlocked a new reward at {company-name}',
            body: `<?php echo File::get(storage_path()."/email-notification/points_spent-editable.html"); ?>`,
            button: {
              text: 'Shop Now',
              color: '#022c82'
            },
          },
          tags: ['{customer}', '{points}', '{points-name}', '{company-name}', '{points-spent-action}', '{reward-name}', '{reward-icon}', '{coupon-code}', '{point-balance}', '{button}'],
          icons: ['reward-icon'],
          config: {
        ...
        methods: {
          ...
          getData: function () {
            let comp = this
            comp.loading = true
            axios.get('/settings/display/email-notifications/points/spent').then((response) => {
          ...
          saveSetting () {
            ...
            if (!comp.saving) {
              ...
              axios.post('/settings/display/email-notifications/points/spent', formData).then((response) => {
                comp.saving = false
          ...
          sendTest () {
            ...
            if (!this.sending) {
              this.sending = true
              axios.post('/api/merchants/' + this.merchantId + '/email-notifications/points/spent/send-test-email', {
                'to_email': this.email
              }).then((response) => {
        ...
        computed: {
          bodyPreview () {
            ...
            var previewText = this.form.body.toString()
              .replace(/{points-spent-action}/g, 'You just redeemed  <strong>{points} {points-name}</strong> for a <strong>{reward-name}</strong> <br/>at {company}. Below is your coupon <br/>code to use on your next purchase.')
              .replace(/{customer}/g, 'Joe Smith')
              .replace(/{points}/g, '500')
              .replace(/{points-name}/g, this.merchantSettings.points.plural_name)
              .replace(/{company(-name)?}/g, this.merchantSettings.name)
              .replace(/{reward-name}/g, (this.merchantSettings.currency.displaySign ? this.merchantSettings.currency.selectedFormat + '10 ' : '10 ' + this.merchantSettings.currency.selectedFormat) + ' off discount')
