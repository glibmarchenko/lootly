            ...
            <div class="row m-t-25 m-b-40">
                        ...
                        <div class="well">
                            <div class="loading-spinner" :class="{ 'loading' : loading || saving }" v-cloak>
                                ...
                                <div class="row m-t-10">
                                    <div class="col-md-12 email-notification-editor">
                                        <label class="bold">
                                            Body
                                        </label>
                                        <trumbowyg
                ...
                <div class="col-md-5 col-12">
                    <div class="sticky-top">
                        <div class="well p-t-15 p-b-15 p-l-0 p-r-0">
                            <div class="border-bottom p-l-15 p-r-15 m-b-10">
                                <h5 class="bold m-b-15 f-s-16">
                                    Preview
                                </h5>
                            </div>
                            <div id="preview-block" class="p-t-15 p-b-15 p-l-25 p-r-25 email-notification-preview">
                                <div class="loading-spinner" :class="{ 'loading' : loading || saving }" v-cloak>
    ...
    <script>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            subjectLine: 'You just received points from {company-name}',
            //body: '<p style="text-align: center;"><span style="font-size: 14px;">{customer}, <br></span></p><p style="text-align: center;"><span style="font-size: 14px;">You just earned <strong>{points} {points-name} </strong>at {company} for {points-earned-action}. </span></p><p><hr></p><h4 class="bold m-b-0" style="text-align: center; line-height: 2;"><span style="font-size: 20px;">Next Reward</span></h4><p style="text-align: center; line-height: normal;"><strong><font color="#7ab74d">{next-reward}</font></strong></p><p style="text-align: center; line-height: normal;"><strong><font color="#7ab74d"><br></font></strong></p><p style="text-align: center;"><font color="#7ab74d">{reward-icon}</font></p><p style="text-align: center;"><font color="#7ab74d"><br></font></p> <p style="text-align: center;">You need <strong>{need-points} more points </strong><br>You have <strong>{point-balance} points</strong></p><p style="text-align: center;"><strong><br></strong></p><p style="text-align: center;">{button}</p><br>',
            body: `<?php echo File::get(storage_path()."/email-notification/points_earned-editable.html"); ?>`,
            button: {
              text: 'Earn More Points',
              color: '#022c82'
            },
          },
          tags: ['{customer}', '{points}', '{points-name}', '{company-name}', '{points-earned-action}', '{next-reward}', '{reward-icon}', '{need-points}', '{point-balance}', '{button}'],
          icons: ['reward-icon'],
          config: {
        ...
        methods: {
          ...
          getData: function () {
            let comp = this
            comp.loading = true
            axios.get('/settings/display/email-notifications/points/earned').then((response) => {
          ...
          saveSetting () {
            ...
            if (!comp.saving) {
              ...
              axios.post('/settings/display/email-notifications/points/earned', formData).then((response) => {
                comp.saving = false
          ...
          sendTest () {
            ...
            if (!this.sending) {
              this.sending = true
              axios.post('/api/merchants/' + this.merchantId + '/email-notifications/points/earned/send-test-email', {
                'to_email': this.email
              }).then((response) => {
        ...
        computed: {
          bodyPreview () {
//            var button = '<a class="btn btn-blue email-button btn-lg" style="background: ' + this.form.button.color + '">' + this.form.button.text + '</a>'
            ...
            var previewText = this.form.body.toString()
              .replace(/{points-earned-action}/g, 'You just earned <strong>{points} {points-name}</strong> at {company-name} for creating an account with us.')
              .replace(/{customer}/g, 'Joe Smith')
              .replace(/{points}/g, '500')
              .replace(/{points-name}/g, 'points')
