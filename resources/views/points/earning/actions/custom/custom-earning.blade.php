...
@section('title', 'Custom Earning Action')
    ...
    <div id="action-page" class="loader m-t-20 m-b-10" v-cloak>
        ...
        <form>
            <div class="row p-b-10 section-border-bottom">
                ...
                <div class="col-md-12">
                    <h3 class="page-title m-t-0 color-dark pull-left">Custom Zapier Action</h3>
                                ...
                                <div class="col-md-8">
                                    <div class="form-group m-b-0">
                                        <label class="light-font m-b-0 m-t-5">
                                            Custom Zapier Action is currently
                                            <span class="bold"
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group m-b-0">
                                        ...
                                        <input class="form-control" name="earning.value"
                                               placeholder="e.g. 50"
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group m-b-0">
                                            <div>
                                                ...
                                                <input class="form-control" placeholder="Custom Earning"
                        ...
                        <div class="well ">
                            <span :class="{'loading' : loading}">
                                ...
                                <div class="preview-action">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <label class="left-preview-icon custom"></label>
      ...
      var page = new Vue({
        ...
        data: {
          ...
          form: {
            ...
            defaultActionName: 'Custom Zapier Action',
            action_slug: 'custom-earning',
            program: {
              ...
              emailDefaultText: 'You just earned <strong>{points} {points-name}</strong> at {company-name} <br/>for {action-name}',
              icon: null,
              icon_name: 'icon-custom',
              action_icon: null,
            },
            zap: '',
            earning: {
          ...
          icon_default_class: 'icon-custom',
          icon_parent_el: 'custom',
          alert: {
            type: '',
            text: '',
            dismissSecs: 5,
        ...
        methods: {
          ...
          bodyPreview () {
            ...
            const previewText = preview
              ...
              .replace(/{action-name}/g, this.form.program.name)
          ...
          getDefaultParches: function () {
            ...
            axios.get('/points/earning/actions/get/' + this.form.defaultActionName).then((response) => {
              if (response.data.action.merchant_action.length === 0 || !this.form.edit) {
                this.form.restrictions.ready = true
              } else {
                  response.data.action.merchant_action.forEach(function (merchant_action, key) {
                      if (merchant_action.id == _this.form.merchant_action_id) {
                          _this.action_id = merchant_action.action_id
                          _this.form.program.icon_name = merchant_action.action_icon_name
                          _this.form.program.name = merchant_action.action_name
                          _this.form.program.status = merchant_action.active_flag
                          _this.form.program.emailDefaultText = merchant_action.reward_email_text
                          _this.form.earning.value = merchant_action.point_value
                          _this.form.restrictions.status = merchant_action.restrictions_enabled
                          _this.form.program.rewardText = merchant_action.reward_text
                          if ((merchant_action.reward_default_text)) {
                              _this.form.program.rewardTextDefault = merchant_action.reward_default_text
                          }
                          _this.form.zap = merchant_action.zap_name
                          _this.form.program.action_icon = merchant_action.action_icon
                          _this.form.emailNotification = merchant_action.send_email_notification === '1' ? true : false
                      }
                  })
          ...
          saveAction: function () {
            this.loading = true
            axios.post('/api/merchants/' + Spark.state.currentTeam.id + '/earning/actions/' + this.form.action_slug + '?edit=' + this.form.edit, this.form).then((response) => {
              this.alert.dismissCountDown = this.alert.dismissSecs
              this.alert.type = 'success'
              this.alert.text = 'Action saved successfully'
              this.form.edit = 1
              this.form.merchant_action_id = response.data.data.id
              this.loading = false
              history.pushState(null, null, '?edit=1&id=' + this.form.merchant_action_id);
            }).catch((error) => {
              this.loading = false
              clearErrors(this.$el)
          ...
          changeRewardText: function () {
            ...
            if (this.form.earning.value > 1) {
              var point = this.form.point_namePlural
            } else {
              var point = this.form.point_name
            }
            this.form.program.rewardPointsName = point
            this.form.program.rewardText = text[0] + ' ' + point
          },
